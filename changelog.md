...
**v1.12.0** - 2026-09-23
- ✨ ADD **Filtrar la vista de año por estado**: el resumen sobre la lista pasa de «Tareas: N · Hechas: N» a «Tareas: N · Hechas: N · Faltantes: N», y las tres palabras son botones. «Hechas» deja solo las tareas hechas, «Faltantes» solo las que faltan y «Tareas» quita el filtro y las enseña todas; volver a tocar el filtro puesto también lo quita. Sirve para el año, un mes o un día, y los números no cambian al filtrar (la palabra activa se marca con el color de acento). Con un filtro puesto el apartado «Se repiten» del año se oculta (las series no tienen estado propio; sus ocurrencias sí se filtran por el estado de cada día en un mes o un día) y, si no queda ninguna tarea de esa clase, sale «Ninguna tarea con este filtro» en lugar de «No hay tareas en este periodo». El filtro se mantiene al cambiar de día, mes o año. Recupera el filtro por estado que se perdió con Tareas pasadas. La lógica está en `filterDaysByStatus` (`utils/year-view.ts`); textos `yearView.summaryTotal`, `summaryDone`, `summaryPending` y `emptyFiltered` en los 4 idiomas (la clave `yearView.summary` ya no se usa)
- 🐛 FIX **El asa de estirar en horizontal salía dentro del calendario, lejos del lomo**: el calendario se dibujaba con una fracción de `flex`, que no repartía el espacio como se calculaba, así que el asa (colocada con el reparto teórico) quedaba a unos 150 dp de las argollas y arrastrarla movía el lomo a otra velocidad que el dedo. Ahora el calendario tiene un ancho explícito (como ya tenía el alto en vertical) y todo —argollas, asa y reparto— sale de ese ancho (`getSideSplit` en `utils/year-view.ts`): el asa va pegada a la derecha de las argollas y sigue al dedo 1 a 1, en móvil y en tablet. El calendario ocupa de verdad lo que marca el reparto: por defecto un tercio en un móvil en horizontal y la mitad en una tablet (antes se veía casi la mitad en el móvil). El reparto guardado se conserva. Las constantes del lomo compacto (`COMPACT_SPINE_WIDTH`, `COMPACT_RING_WIDTH`) pasan a `bookStyles`
- 🔧 IMPROVE **Argollas más separadas y asa más visible en horizontal**: en la vista de año las argollas van más separadas que en el libro (32 dp en el móvil y 1,4 veces la distancia normal en tablet) y el asa es una barra más grande (5×48 dp) sobre una franja de 24 dp para agarrarla desde cualquier punto del lomo. `BookSpine` admite una separación propia (`ringPitch`); el libro de la agenda no cambia
- 🔧 IMPROVE Tests nuevos del filtro por estado y del reparto en horizontal (`filterDaysByStatus`, `getSideSplit` y su cableado en la vista); se adaptaron las dos comprobaciones del resumen antiguo

**v1.11.0** - 2026-09-21
- ✨ ADD **Barra de abajo renovada**: ahora es Agenda, Calendario, Notas, Recordatorios y Ajustes. "Calendario" abre la vista de año y "Notas" la sección de notas, así que ya no hace falta buscarlas en la cabecera. Las tres son la misma pantalla (no se monta nada duplicado) y el botón marcado sigue lo que estás viendo, también al llegar desde Ajustes o Recordatorios (`components/section-tabs.tsx`). Como Expo Router no admite `href` junto a un botón propio, esas tres pestañas no lo llevan; sin sesión la barra entera ya va oculta
- ✨ ADD **Tareas nuevas desde la vista de año**: un botón ＋ a la derecha del título cuando hay un día seleccionado, y otro en la cabecera de cada fecha de las listas de un mes o del año. Abre el editor con una tarea nueva en la primera línea libre de ese día (si están todas ocupadas abre una línea extra, con el tope de 20) y se guarda sin salir de la vista. Solo sale de hoy en adelante: los días pasados no llevan el botón. Textos "Añadir tarea" en los 4 idiomas
- ✨ ADD **El número del día del libro es un atajo**: tocarlo lleva a la vista de año con ese día seleccionado y sus tareas en la lista
- ✨ ADD **Botones con iconos en el editor de tareas, en el de notas y en el reporte de errores**: Cancelar (X), Eliminar (papelera), Marcar como completada (check; deshacer si ya lo está, solo en tareas) y Guardar (disquete) pasan de texto a icono, y en el reporte de errores Enviar es un avión de papel. Mientras se envía el reporte, un velo con spinner y "Enviando…" cubre el formulario y evita tocarlo. Conservan su etiqueta de accesibilidad
- 🔧 IMPROVE **Las tareas de la vista de año se editan en el sitio**: antes, tocar una (o una serie) montaba el libro entero, saltaba de página y a veces giraba la pantalla con el modal abriéndose a la vez, y tardaba mucho en abrir, guardar y cerrar. Ahora el editor se abre sin salir del año. El modal con toda su lógica de guardar y borrar (tareas normales, repetidas y series) es un único componente, `DayTaskEditModal`, que usan el libro y el año. La página del libro también deja de suscribirse a todo el store de tareas, así que se re-dibuja menos al guardar
- 🔧 IMPROVE **Widgets más simples**: tocar una tarea del widget de tareas lleva el libro a ese día sin abrir la tarea, y tocar una nota (o el tablero) del widget de notas abre la sección de notas sin abrir ninguna nota. Abrir el editor nada más llegar era lo que se atascaba. Los botones ＋ de los dos widgets siguen abriendo una tarea o nota nueva
- 🐛 FIX **Fotos verticales del widget de notas**: salían tumbadas porque el widget ignoraba la orientación EXIF que guarda la cámara. Ahora se giran o voltean como se hicieron (todas las orientaciones) y siguen recortándose para llenar el post-it, igual que en la app
- 🐛 FIX En modo oscuro el botón "Añadir" del gestor de tipos era blanco sobre blanco (usaba el color `tint`, blanco en oscuro, con texto blanco); ahora usa el color de acento y el de su texto, como el resto de botones
- 🐛 FIX La tira de tipos de tarea ya no aparece en Notas: las notas no son tareas y no tienen tipos
- 🔧 IMPROVE La cabecera de la agenda solo lleva los tipos (Todas, Sin tipo, uno por tipo y el ＋): el conmutador Libro/Año y la pestaña Notas se quitaron porque ya están en la barra de abajo
- 🔧 IMPROVE Los iconos inactivos de la barra usan el color del tema (`tabIconDefault`), así que Recordatorios y Ajustes cambian un poco de gris
- 🗑️ REMOVE **Tareas pasadas**: la pestaña y su carpeta (~900 líneas) se eliminan; la vista de año ya enseña las tareas de cualquier año, mes o día, con las ocurrencias de las repetidas (que Tareas pasadas ignoraba, y por eso su porcentaje de completado no era fiable). Se pierden el filtro por estado (hechas/pendientes) y esas estadísticas
- 🗑️ REMOVE **El calendario modal** que se abría con el icono junto al número del día (`anotherCalendarModal`, `DayDetailModal`, `QuickAddTaskButton` y sus estilos) y `Calendar-store`, que solo guardaba su fecha. La dependencia `react-native-calendars` sigue instalada. La clave `calendar-storage` queda huérfana en el almacenamiento del móvil (unos bytes)
- 🔧 IMPROVE Se borraron los tests de lo eliminado (`getFilteredPastTasks`, la pestaña Notas y los botones Libro/Año de la cabecera, los toques del widget que abrían tareas y notas concretas)
